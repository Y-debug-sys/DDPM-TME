...
class ER(nn.Module):
    def __init__(self, in_dim=529, hidden_size=144, out_dim=529):
...
class Base(nn.Module):
    def __init__(self, dim, init_dim=None, out_dim=None, dim_mults=(1, 2, 4), channels=3, resnet_block_groups=8):
        super().__init__()
...
class DDPM(nn.Module):
    ...
    def __init__(self, model, seq_length, timesteps=1000, sampling_timesteps=None, loss_type='l1', ddim_sampling_eta=0.,
    ...
    def p_sample_loop(self, shape):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            tm, _, noise = self.p_sample(tm, t)
            noise_seq.append(noise)

        tm = self.unnormalize(tm)
        tm = tm.reshape(-1, self.seq_length * self.channels)
    ...
    def ddim_sample(self, shape, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(tm, time_cond, clip_x_start=clip_denoised)
    ...
    def opt_sample(self, noise_set, clip_denoised=True):
        i = 0
        tm = noise_set[:, i, :, :]

        batch, device, total_timesteps, sampling_timesteps, eta = \
            tm.shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta
    ...
    def forward(self, tm, *args, **kwargs):
        b, c, n, device, seq_length, = *tm.shape, tm.device, self.seq_length
        assert n == seq_length, f'seq length must be {seq_length}'

        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        tm = self.normalize(tm)

        return self.p_losses(tm, t, *args, **kwargs)
